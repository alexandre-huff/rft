// :vim ts=4 sw=4 noet:
/*
==================================================================================
	Copyright (c) 2020 AT&T Intellectual Property.

	Licensed under the Apache License, Version 2.0 (the "License");
	you may not use this file except in compliance with the License.
	You may obtain a copy of the License at

		http://www.apache.org/licenses/LICENSE-2.0

	Unless required by applicable law or agreed to in writing, software
	distributed under the License is distributed on an "AS IS" BASIS,
	WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
	See the License for the specific language governing permissions and
	limitations under the License.
==================================================================================
*/

/*
	Mnemonic:	test_log.cpp
	Abstract:	Tests the RFT log module

	Date:		2 September 2020
	Author:		Alexandre Huff
*/

#include "CppUTest/TestHarness.h"
#include "CppUTest/TestHarness_c.h"
#include "CppUTestExt/MockSupport.h"


extern "C" {
	#include <string.h>
	#include "types.h"
	#include "log.h"
	#include "mtl.h"
	#include "stubs/stub_logger.h"
}

TEST_GROUP( TestLog ) {
	log_entry_t *entry;     // generic log entry pointer
	const char *server = "server1";
	const char *target = "127.0.0.1:4560";
	server_conf_cmd_data_t config_data;	// RAFT_CONFIG data

	void setup() {
		entry = NULL;

		// initilizing raft log entries for using on almost all tests
		snprintf( config_data.server_id, sizeof( server_id_t ), "%s", server );
		snprintf( config_data.target, sizeof( server_id_t ), "%s", target );
	}

	void teardown() {
		mock().clear();
	}
};

TEST( TestLog, NewRaftConfigLogEntry ) {
	server_conf_cmd_data_t entry_data;	// RAFT_CONFIG data from entry

	cpputest_malloc_set_out_of_memory();
	entry = new_raft_log_entry( 1, RAFT_CONFIG, ADD_MEMBER, &config_data, sizeof(server_conf_cmd_data_t) );
	CHECK( entry == NULL );
	cpputest_malloc_set_not_out_of_memory();

	entry = new_raft_log_entry( 1, RAFT_CONFIG, ADD_MEMBER, &config_data, sizeof(server_conf_cmd_data_t) );
	CHECK( entry != NULL );

	LONGS_EQUAL( 1, entry->term );
	LONGS_EQUAL( RAFT_CONFIG, entry->type );
	LONGS_EQUAL( ADD_MEMBER, entry->command );
	LONGS_EQUAL( sizeof(server_conf_cmd_data_t), entry->dlen );

	memcpy( &entry_data, entry->data, entry->dlen );
	STRCMP_EQUAL( server, entry_data.server_id );
	STRCMP_EQUAL( target, entry_data.target );

	POINTERS_EQUAL( NULL, entry->context );
	POINTERS_EQUAL( NULL, entry->key );

	free_log_entry( entry );
}

TEST( TestLog, NewRaftCommandLogEntry ) {
	int command = 0;
	int cmd_data = 1;
	int entry_data;

	cpputest_malloc_set_out_of_memory();
	entry = new_raft_log_entry( 1, RAFT_COMMAND, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry == NULL );
	cpputest_malloc_set_not_out_of_memory();

	entry = new_raft_log_entry( 1, RAFT_COMMAND, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry != NULL );

	LONGS_EQUAL( 1, entry->term );
	LONGS_EQUAL( RAFT_COMMAND, entry->type );
	LONGS_EQUAL( command, entry->command );
	LONGS_EQUAL( sizeof(cmd_data), entry->dlen );

	memcpy( &entry_data, entry->data, entry->dlen );
	LONGS_EQUAL( cmd_data, entry_data );

	POINTERS_EQUAL( NULL, entry->context );
	POINTERS_EQUAL( NULL, entry->key );

	free_log_entry( entry );
}

TEST( TestLog, NewServerLogEntry ) {
	int command = 1;
	int cmd_data = 50;
	int entry_data;
	char context[16];
	char key[16];

	snprintf( context, sizeof(context), "%s", "mycontext" );
	snprintf( key, sizeof(key), "%s", "mykey" );

	cpputest_malloc_set_out_of_memory();
	entry = new_server_log_entry( context, key, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry == NULL );
	cpputest_malloc_set_not_out_of_memory();

	entry = new_server_log_entry( context, key, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry != NULL );

	// LONGS_EQUAL( 0, entry->term );   // does not matter for server log entry
	LONGS_EQUAL( SERVER_COMMAND, entry->type );
	LONGS_EQUAL( command, entry->command );
	LONGS_EQUAL( sizeof(cmd_data), entry->dlen );

	memcpy( &entry_data, entry->data, entry->dlen );
	LONGS_EQUAL( cmd_data, entry_data );

	STRCMP_EQUAL( context, entry->context );
	STRCMP_EQUAL( key, entry->key );

	free_log_entry( entry );
}

TEST( TestLog, NewServerLogEntry_NoKey_NoContext ) {
	int command = 1;
	int cmd_data = 50;
	char context[8];
	char key[8];

	memset(context, 0, 8 );
	memset(key, 0, 8 );

	// testing with NULL context and NULL key
	entry = new_server_log_entry( NULL, NULL, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry == NULL );

	// testing with empty context and key
	entry = new_server_log_entry( context, key, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry == NULL );

	snprintf( context, sizeof(context), "ctx" );	// setting a context

	// testing with NULL key
	entry = new_server_log_entry( context, NULL, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry == NULL );

	// testing with empty key
	entry = new_server_log_entry( context, key, command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry == NULL );
}

TEST( TestLog, GetRaftLogEntry_EmptyLog ) {
	entry = get_raft_log_entry( 0 );
	CHECK( entry == NULL );
}

TEST( TestLog, GetServerLogEntryFromEmptyLog ) {
	entry = get_server_log_entry( 0, &config_data.server_id );
	CHECK( entry == NULL );
}

TEST( TestLog, AppendRaftLogEntry ) {
	log_entries_t *raft_log = NULL;

	mock()
		.expectOneCall( "raft_config_get_server" )
		.withParameter( "server_id", config_data.server_id )
		.andReturnValue( (void *)NULL );
	mock()
		.expectOneCall( "raft_config_add_server" )
		.withParameter( "server_id", config_data.server_id )
		.withParameter( "target", config_data.target )
		.withParameter( "last_log_index", 0 )
		.andReturnValue( 1 );
	mock()
		.expectOneCall( "raft_config_set_server_status" )
		.withParameter( "server_id", config_data.server_id )
		.withParameter( "status", VOTING_MEMBER )
		.andReturnValue( 1 );

	entry = new_raft_log_entry( 1, RAFT_CONFIG, ADD_MEMBER, &config_data, sizeof(server_conf_cmd_data_t) );
	CHECK( entry != NULL );
	append_raft_log_entry( entry );

	mock().checkExpectations();

	raft_log = get_raft_log( );
	CHECK( raft_log != NULL );
	LONGS_EQUAL( 1, raft_log->head );	// we added the first log entry
	LONGS_EQUAL( INITIAL_LOG_ENTRIES, raft_log->size );

	free( raft_log->entries );
	free_log_entry( entry );
}

TEST( TestLog, AppendServerLogEntry ) {
	log_entries_t *server_log;
	int command = 1;
	int cmd_data = 50;

	entry = new_server_log_entry( "myctx", "mykey", command, &cmd_data, sizeof(cmd_data) );
	CHECK( entry != NULL );
	append_server_log_entry( entry, &config_data.server_id );

	server_log = get_server_log( &config_data.server_id );
	CHECK( server_log != NULL );
	LONGS_EQUAL( 1, server_log->head );	// we added the first log entry
	LONGS_EQUAL( INITIAL_LOG_ENTRIES, server_log->size );

	free( server_log->entries );
	free( get_xapps_logs( )->servers_logs );
	free_log_entry( entry );
}

/* ===================================== Next group of tests ===================================== */
/*
	This group of tests will test all functions that require log entries appended to the corresponding logs
*/
TEST_GROUP( TestLogInitialized ) {
	raft_state_t me;
	log_entry_t *entry1;	// generic log entry pointer
	log_entry_t *entry2;	// generic log entry pointer
	const char *server1 = "server1";
	const char *server2 = "server2";
	const char *target1 = "127.0.0.1:4560";
	const char *target2 = "127.0.0.2:4560";
	server_conf_cmd_data_t config_data1;	// RAFT_CONFIG data of server1
	server_conf_cmd_data_t config_data2;	// RAFT_CONFIG data of server2

	void setup() {
		entry1 = NULL;
		entry2 = NULL;

		me.commit_index = 0;

		// initilizing raft log entries for using on almost all tests
		snprintf( config_data1.server_id, sizeof(server_id_t), "%s", server1 );
		snprintf( config_data1.target, sizeof(server_id_t), "%s", target1 );
		snprintf( config_data2.server_id, sizeof(server_id_t), "%s", server2 );
		snprintf( config_data2.target, sizeof(server_id_t), "%s", target2 );

		mock().disable();	// disabling mock checks (but it still returns the default mock value)
		entry1 = new_raft_log_entry( 1, RAFT_CONFIG, ADD_MEMBER, &config_data1, sizeof(server_conf_cmd_data_t) );	// server1
		CHECK( entry1 != NULL );
		append_raft_log_entry( entry1 );

		entry2 = new_raft_log_entry( 1, RAFT_CONFIG, ADD_MEMBER, &config_data2, sizeof(server_conf_cmd_data_t) );	// server1
		CHECK( entry2 != NULL );
		append_raft_log_entry( entry2 );
		mock().enable();
	}

	void teardown() {
		mock().clear();

		log_entries_t *raft_log = get_raft_log( );
		if( raft_log->head > 0 ) {
			mock().disable();	// disabling mock checks (but it still returns the default mock value)
			remove_raft_conflicting_entries( 1, &me );
			mock().enable();
		}

		if( raft_log->entries ) {
			free( raft_log->entries );
			raft_log->entries = NULL;
			raft_log->head = 0;
			raft_log->size = 0;
		}
	}
};

TEST( TestLogInitialized, RemoveAllRaftConflictingEntries ) {
	int removed;

	mock()
		.expectOneCall( "raft_config_remove_server" )
		.withParameter( "server_id", config_data2.server_id );

	mock()
		.expectOneCall( "raft_config_remove_server" )
		.withParameter( "server_id", config_data1.server_id );

	// removing all log entries ( from the first one )
	removed = remove_raft_conflicting_entries( 1, &me );
	CHECK_TRUE( removed );

	mock().checkExpectations();

	log_entries_t *raft_log = get_raft_log( );
	CHECK_EQUAL( 0, raft_log->head );
}

TEST( TestLogInitialized, RemoveTheLastConflictingEntry ) {
	int removed;

	mock()
		.expectOneCall( "raft_config_remove_server" )
		.withParameter( "server_id", config_data2.server_id );

	// removing the second log entry
	removed = remove_raft_conflicting_entries( 2, &me );
	CHECK_TRUE( removed );

	mock().checkExpectations();

	log_entries_t *raft_log = get_raft_log( );
	CHECK_EQUAL( 1, raft_log->head );
}

/*
	Trying to remove a committed log index, i.e. from_index <= commit_index
*/
TEST( TestLogInitialized, RemoveRaftConflictingEntries_IndexLTCommitIndex ) {
	int removed;

	me.commit_index = 2;
	removed = remove_raft_conflicting_entries( 2, &me );	// equal
	CHECK_FALSE( removed );

	removed = remove_raft_conflicting_entries( 1, &me );	// lower than
	CHECK_FALSE( removed );

	log_entries_t *raft_log = get_raft_log( );
	CHECK_EQUAL( 2, raft_log->head );

	me.commit_index = 0;	// allowing tear down remove all log entries
}

TEST( TestLogInitialized, RemoveRaftConflictingEntries_FromIndexZero ) {
	int removed;

	removed = remove_raft_conflicting_entries( 0, &me );
	CHECK_FALSE( removed );

	log_entries_t *raft_log = get_raft_log( );
	CHECK_EQUAL( 2, raft_log->head );
}

TEST( TestLogInitialized, RemoveRaftConflictingEntries_IndexGreaterThanHead ) {
	int removed;

	removed = remove_raft_conflicting_entries( 100, &me );
	CHECK_FALSE( removed );

	log_entries_t *raft_log = get_raft_log( );
	CHECK_EQUAL( 2, raft_log->head );
}

/*
	Removing a DEL_MEMBER conflicting log entry actually adds it back to
	the raft configuration
*/
TEST( TestLogInitialized, RemoveRaftConflictingEntries_DeleteMember ) {
	log_entry_t *del_entry;
	server_conf_cmd_data_t del_server;
	log_entries_t *raft_log;

	snprintf( del_server.server_id, sizeof(server_id_t), "server3" );
	snprintf( del_server.target, sizeof(server_id_t), "127.0.0.3:4560" );

	del_entry = new_raft_log_entry( 1, RAFT_CONFIG, DEL_MEMBER, &del_server, sizeof(server_conf_cmd_data_t) );
	CHECK( del_entry != NULL );
	mock().disable();
	append_raft_log_entry( del_entry );
	mock().enable();

	raft_log = get_raft_log( );
	LONGS_EQUAL( 3, raft_log->head );

	mock()
		.expectOneCall( "raft_config_add_server" )
		.withParameter( "server_id", del_server.server_id )
		.withParameter( "target", del_server.target )
		.withParameter( "last_log_index", 0 )
		.andReturnValue( 1 );

	mock()
		.expectOneCall( "raft_config_set_server_status" )
		.withParameter( "server_id", del_server.server_id )
		.withParameter( "status", VOTING_MEMBER )
		.andReturnValue( 1 );

	// removing the this DEL_MEMBER conflicting log entry
	int removed = remove_raft_conflicting_entries( 3, &me );
	mock().checkExpectations();

	CHECK_TRUE( removed );
	LONGS_EQUAL( 2, raft_log->head );
}

TEST( TestLogInitialized, GetRaftLogEntry_LogInitialized ) {
	log_entry_t *entry;

	entry = get_raft_log_entry( 0 );
	CHECK_FALSE( entry );

	entry = get_raft_log_entry( 3 );
	CHECK_FALSE( entry );

	entry = get_raft_log_entry( 2 );
	CHECK_TRUE( entry );
	LONGS_EQUAL( 2, entry->index );
}

TEST( TestLogInitialized, GetRaftLastLogIndex ) {
	index_t index;

	index = get_raft_last_log_index( );
	CHECK_TRUE( index );
	LONGS_EQUAL( 2, index );
}

TEST( TestLogInitialized, GetRaftLastLogTerm ) {
	term_t term;

	term = get_raft_last_log_term( );
	CHECK_TRUE( term );
	LONGS_EQUAL( 1, term );
}

TEST( TestLogInitialized, SerializeRaftLogEntries ) {
	unsigned int bytes;
	unsigned int esize = 0;		// size of the serialized log entries
	unsigned char *buf = NULL;	// the serialize function will do memory allocation
	unsigned int buf_len = 0;	// the serialize function will populate this value
	unsigned int n_entries = 2;

	/*
		TODO needs to add a test with insufficient message size (1 byte)
		This requires to change the append_entries function to rely on the
		number of bytes instead on the n_entries to decide if the message
		should contain serialized log entries or just a heartbeat request
	*/
	// bytes = serialize_raft_log_entries( 1, &n_entries, &buf, &buf_len, 1 );
	// LONGS_EQUAL( 0, bytes );

	esize += RAFT_LOG_ENTRY_HDR_SIZE + entry1->dlen;
	esize += RAFT_LOG_ENTRY_HDR_SIZE + entry2->dlen;

	// testing with the same message size than the serialized log entries size + append entries header length
	bytes = serialize_raft_log_entries( 1, &n_entries, &buf, &buf_len, esize + APND_ENTR_HDR_LEN );
	LONGS_EQUAL( esize, bytes );

	if( buf )
		free( buf );
}


TEST( TestLogInitialized, DeserializeRaftLogEntries ) {
	unsigned int bytes;
	unsigned int esize = 0;		// size of the serialized log entries
	unsigned char *buf = NULL;	// the serialize function will do memory allocation
	unsigned int buf_len = 0;	// the serialize function will populate this value
	unsigned int n_entries = 2;
	log_entry_t **entries = NULL;
	log_entry_t *exp[2];	// expected log entry
	server_conf_cmd_data_t *exp_cmd_data;	// expected data
	server_conf_cmd_data_t *e_data;			// entry data

	esize += RAFT_LOG_ENTRY_HDR_SIZE + entry1->dlen;
	esize += RAFT_LOG_ENTRY_HDR_SIZE + entry2->dlen;
	//	max_msg_size if the sum of all log entries size (esize) and the append entries header length
	bytes = serialize_raft_log_entries( 1, &n_entries, &buf, &buf_len, esize + APND_ENTR_HDR_LEN );

	entries = (log_entry_t **) malloc( n_entries * sizeof(log_entry_t *) );
	CHECK( entries != NULL );
	memset( entries, 0, n_entries * sizeof(log_entry_t *) );

	deserialize_raft_log_entries( buf, n_entries, entries );
	/*
		IMPORTANT: in case of errors, they also might checked at the
		serialization function, since it can has a particular bug.
		We do not test data of the serialized log, as we are deserializing
		here and thus, this test is executed here (no duplicated tests).
	*/
	if( n_entries > 2 )	// this only makes sure the we have setup all entries we have serialized
		FAIL( "The number of entries in setup/teardown functions differ from n_entries" );

	exp[0] = entry1;	// unfortunately, we have to add entries to the expected array manually
	exp[1] = entry2;

	for( int i = 0; i < n_entries; i++ ) {
		LONGS_EQUAL( exp[i]->term, entries[i]->term );
		LONGS_EQUAL( exp[i]->index, entries[i]->index );
		LONGS_EQUAL( exp[i]->dlen, entries[i]->dlen );
		LONGS_EQUAL( exp[i]->command, entries[i]->command );
		LONGS_EQUAL( exp[i]->type, entries[i]->type );

		if( entries[i]->type == RAFT_CONFIG ) {
			CHECK( entries[i]->context == NULL );
			CHECK( entries[i]->key == NULL );
			LONGS_EQUAL( 0, entries[i]->clen );
			LONGS_EQUAL( 0, entries[i]->klen );
			exp_cmd_data = (server_conf_cmd_data_t *) exp[i]->data;
			e_data = (server_conf_cmd_data_t *) entries[i]->data;
			STRCMP_EQUAL( exp_cmd_data->server_id, e_data->server_id );
			STRCMP_EQUAL( exp_cmd_data->target, e_data->target );
		} // TODO needs to implement tests for server deserialization: else if( entries[i]->type == SERVER_COMMAND ){}

		free_log_entry( entries[i] );
	}
	if( entries )
		free( entries );
	if( buf )
		free( buf );
}

/*
	TODO tests to implement:
	- get_server_log_entry - TODO
	- get_server_last_log_index - TODO
	- serialize_server_log_entries - TODO
	- serialize_log_entries (called by both raft and server serialize functions) - TODO
	- deserialize_log_entries (needs to implement server log entries deserialization)
*/
